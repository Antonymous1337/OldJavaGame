...
public class projectSend extends PApplet {
...
public void setup() {
  ...
  e = new String[1];
  e[0] = "Bullet size: " + player.bulletSize + " + 2.5 = " + (player.bulletSize+2.5f);
  ...
  e = new String[1];
  e[0] = "Rocket drift: " + player.rocketDrift + " + " + 0.2f + " = " + (player.rocketDrift+0.2f);
  ...
  e = new String[1];
  e[0] = "shield regeneration: " + player.shieldRegen + " + " + 0.004f + " = " + (player.shieldRegen+0.004f);
  ...
  e = new String[1];
  e[0] = "upgrade point drop multiplier: " + player.upgradePointMultiplier + " + " + 0.2f + " = " + (player.upgradePointMultiplier+0.2f);
  upgrades[2][0] = new upgrade(n, d, e, 8500, 14);
  
  n = "quicker level progression";
  d = "Increases the amount of upgrade points dropped";
  e = new String[1];
  e[0] = "level progression multiplier: " + player.levelMultiplier + " + " + 0.2f + " = " + (player.levelMultiplier+0.5f);
...
class centerxyBoss extends enemyBoss {
  ...
  public void shoot() {
    ...
    if (attackVersion == 1) {
      ...
      if (i%20 == 0) bullets.add(new bullet(x, y, DMG*1.5f, 10, 3, random(0, 360), false, bc, gc));
      ...
      if (i%90==0) {
        ...
        for (int j = 0; j < 49; j++) bullets.add(new bullet(x, y, DMG*1.2f, 10, 5, deg+j-29, false, bc, gc));
      }
  ...
  public boolean checkCollision(float x, float y, float size) {
  ...
  public void shoot() {
    ...
    if (attackVersion == 1) {
      ...
      if (i%90==45) {
        ...
        for (int j = 0; j < 23; j++) bullets.add(new bullet(x-400, y+200, DMG*1.2f, 10, 5, deg+j-11, false, bc, gc));
      }
      
      if (i%4 == 0) bullets.add(new bullet(x+200, y+100, DMG/1.2f, 8, 6, 135 + random(0, 90), false, bc, gc));
      if (i%4 == 2) bullets.add(new bullet(x-200, y+100, DMG/1.2f, 8, 6, 135 + random(0, 90), false, bc, gc));
  ...
  public void shoot() {
    ...
    if (attackVersion == 1) {
      ...
      if (i == 0) {
        ...
        for (int j = 0; j < 20; j++) bullets.add(new bullet(x-45, y-90, DMG, 6, 6+random(-0.5f, 0.5f), deg-15+random(0,30), false, bc, gc));
      } else if (i == 44) {
        boolean right = false;
        float a = (this.x+45) - player.x; // x between ship and player
        float b = (this.y+90) - player.y; // y betwwen ship and player
      ...
      for (int j = 0; j < 3; j++) {
        if (counter[j] != 0) {
          ...
          if (counter[j] < 126-35) {
            ...
            if (counter[j] < 5) for (int k = 0; k < 16; k++) bullets.add(new bullet(rememberX[j], rememberY[j], DMG/5, 10, 3, k*22.5f, false, bc, gc));
          }
        }
      }
...
class bullet {  
  ...
  bullet(float xx, float yy, float ndamage, float nsize, float nspeed, float ndegrees, boolean pb, int bc, int gc) {
    x = xx;
    y = yy;
    damage = ndamage;
    size = nsize;
...
class button {
  ...
  int x, y, l, w;
  int c1, c2; // hover, not hovered
  String s;
  
  boolean selected = false;
  
  button(int xx, int yy, int ll, int ww, int cc1, int cc2, String ss, int ID) {
    x = xx;
    y = yy;
    l = ll;
    w = ww;
...
class enemyBoss {
  ...
  public boolean checkCollision(float x, float y, float size) {return false;}
}
...
public void spawnEnemies() {
  ...
  if (bombIt == 0 && (frameCount%(300/spawnRate)==0 || enemies.size() == 0)) {
    int r = (int)random(0, 100);
    if (r < 60) {
      enemies.add(new enemyNormalShip(baseHP, baseDMG, 2.5f));
    } else if (r < 90){
      enemies.add(new enemyBigShip(baseHP, baseDMG, 1));
    } else {
      enemies.add(new enemyHealShip(baseHP, baseDMG, 1.5f));
    }
...
public void checkLevelUp() {
  if (backgroundScore >= levelIncrease) {
    if (!bossPractice) {
      level++;
      backgroundScore = 0;
      levelIncrease = (int) (levelIncrease * 1.5f);
      baseHP *= di;
      baseDMG *= di;
      baseReward *= 1.3f;
...
class playerShip {
  ...
  playerShip() {
    ...
    if (di == 1.2f) difficultyScoreChanger = 0.5f;
    else if (di == 1.3f) difficultyScoreChanger = 2;
    else difficultyScoreChanger = 8;
    
    if (di == 1.2f) difficultyRewardChanger = 0.75f;
    else if (di == 1.3f) difficultyRewardChanger = 1;
    else difficultyRewardChanger = 1.5f;
    
    playerMoved = false;
  }
  
  public void giveReward(float s, float bs, float up) {
    score += ceil((s*levelMultiplier*difficultyScoreChanger)/1.3f);
    backgroundScore += ceil(bs*levelMultiplier);
    upgradePoints += ceil((up*levelMultiplier*upgradePointMultiplier/difficultyRewardChanger)*1.3f);
  }
  ...
  public void displayShip() {
    ...
    fill(0, 255, 0);
    rect(x-20, y+32.5f, map(curHealth, 0, maxHealth, 0, 40), 2.5f);
    fill(0, 200, 200);
    rect(x-20, y+35, map(curShield, 0, maxShield, 0, 40), 1.5f);
    if (curShield > 0) {
  ...
  public void shoot() { // add a for loop to determine this
    if (bulletCooldown <= 0) {
...
class enemyNormalShip extends enemyShip {
  ...
  public void drawEnemy() {
    ...
    */
    ...
    fill(255, 0, 0);
    rect(x-20, y-25, map(HP, 0, initialHP, 0, 40), 2.5f);
  }
...
class enemyHealShip extends enemyShip {
  ...
  public void drawEnemy() {
    image(healShip, x-17.5f, y-17.5f, 35, 35);
    strokeWeight(1);
    stroke(255);
    noFill();
    square(x-17.5f, y-17.5f, 35);
    noStroke();
    /*
    fill(255, 0, 0);
    circle(x, y, 35);
    ...
    fill(255, 0, 0);
    rect(x-17.5f, y-22.5f, map(HP, 0, initialHP, 0, 35), 2.5f);
  }
  ...
  public void giveReward() {
    player.giveReward(baseReward*1.5f, baseReward*1.5f, baseReward*1.5f);
  }
  
  public void checkImpact() {
    if (this.y > 1000) oob = true;
    for (bullet b : bullets) {
      if (this.x-17.5f < b.x+(b.size/2) && b.x-(b.size/2) < this.x+17.5f && this.y-17.5f < b.y+(b.size/2) && b.y-(b.size/2) < this.y+17.5f && b.playerBullet) {
        b.oob = true;
        this.damage(b.damage);
        if (this.oob) break;
      }
  ...
  // assumes x/y to be center
  public boolean checkCollision(float x, float y, float size) {
    if (this.x-17.5f < x+(size/2) && x-(size/2) < this.x+17.5f && this.y-17.5f < y+(size/2) && y-(size/2) < this.y+17.5f) return true;
    else return false;
  }
...
class enemyBigShip extends enemyShip {
  ...
  public void shoot() {
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 0, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 45, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 90, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 135, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 180, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 225, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 270, false, bc, gc));
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, 315, false, bc, gc));
  }
  ...
  public void drawEnemy() {
    ...
    fill(255, 0, 0);
    rect(x-25, y-30, map(HP, 0, initialHP, 0, 50), 2.5f);
  }
...
class sniper extends enemyShip {
  ...
  public void drawEnemy() {
    ...
    fill(255, 0, 0);
    rect(x-20, y-30, map(HP, 0, initialHP, 0, 40), 2.5f);
...
class scatter extends enemyShip {
  ...
  int i = 0;
  public void shoot() {
    if (i == 0) for (int j = 0; j < 8; j++) bullets.add(new bullet(x, y, DMG, 10, 2.5f, 45*j, false, bc, gc));
    else if (i == 1) for (int j = 0; j < 8; j++) bullets.add(new bullet(x, y, DMG, 10, 2.5f, 30+(45*j), false, bc, gc));
    else if (i == 2) for (int j = 0; j < 8; j++) bullets.add(new bullet(x, y, DMG, 10, 2.5f, 60+(45*j), false, bc, gc));
...
class rando extends enemyShip {
  ...
  public void shoot() {
    bullets.add(new bullet(x, y, DMG, 10, 2.5f, random(0, 360), false, bc, gc));
  }
  ...
  public void drawEnemy() {
    ...
    fill(255, 0, 0);
    rect(x-30, y-35, map(HP, 0, initialHP, 0, 50), 2.5f);
...
class carrier extends enemyShip {
  ...
  public void drawEnemy() {
    ...
    fill(255, 0, 0);
    rect(x-50, y-25, map(HP, 0, initialHP, 0, 100), 2.5f);
...
class missleMaster extends enemyShip {
  ...
  public void shoot() {
    for (int i = 0; i < 16; i++) missles.add(new missle(x, y, DMG, 4, 3, deg+(22.5f*i), false, 45, 1.5f, 0));
  }
...
class upgrade {
  ...
  public void setArea(float xx, float yy, float ll, float ww) {
    x = xx;
    y = yy;
    l = ll;
    w = ww;
      ...
      if (upgradePoints >= cost){
        ...
        switch (id) {
          ...
          case 4:
            ...
            player.bulletSize += 2.5f;
            if (player.bulletSize >= 12) maxed = true;
            
            cost *= 10;
            
            effects[0] = "Bullet size: " + player.bulletSize + " + 2.5 = " + (player.bulletSize+2.5f);
          ...
          case 8:
            ...
            player.rocketDrift += 0.2f;
            if (player.rocketDrift >= 2) maxed = true;
            
            cost *= 7;
          
            effects[0] = "Rocket drift: " + player.rocketDrift + " + " + 0.2f + " = " + (player.rocketDrift+0.2f);
          ...
          case 11:
            ...
            if (player.shieldRegen == 0.001f) player.shieldRegen += 0.004f;
            else {
              player.shieldRegen += 0.005f;
              maxed = true;
            }
            
            cost *= 7;
          
            effects[0] = "shield regeneration: " + player.shieldRegen + " + " + 0.005f + " = " + (player.shieldRegen+0.005f);
          ...
          case 14:
            ...
            player.upgradePointMultiplier += 0.2f;
            if (player.upgradePointMultiplier >= 2) maxed = true;
            
            cost *= 5;
          
            effects[0] = "upgrade point drop multiplier: " + player.upgradePointMultiplier + " + " + 0.2f + " = " + (player.upgradePointMultiplier+0.2f);
          
            break;  
          case 15:
          
            player.levelMultiplier += 0.5f;
            if (player.levelMultiplier >= 4) maxed = true;
            
            cost *= 4;
          
            effects[0] = "level progression multiplier: " + player.levelMultiplier + " + " + 0.2f + " = " + (player.levelMultiplier+0.5f);
...
*/
  public void settings() {  size(1400, 1000, P2D);  smooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "projectSend" };
    if (passedArgs != null) {
